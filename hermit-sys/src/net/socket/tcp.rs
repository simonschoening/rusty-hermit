...
use crate::net::{executor, local_endpoint, socket_map};
...
impl AsyncTcpSocket {
		...
		let (new_state, error) = match self.state.load() {
			...
			state @ State::Connecting => self.with_socket_ref(|socket| {
				if socket.may_send() && socket.may_recv() {
					...
					debug!(
						"conection refused, socket in state {:?}",
						self.with_socket_ref(|tcp| tcp.state())
					);
					(
						State::Closed,
						Some(io::Error::new(
                        ...
                        if let Some(socket) = self.socket {
                            ...
                            nic::lock().with(|nic|
                                nic.with_tcp_socket_mut(self.handle,|tcp_socket| {
                                    let waker = cx.waker();
                                    if wake == WakeOn::Recv || wake == WakeOn::SendRecv {
                                        trace!("AsyncTcpSocket with {:?} is Pending and wakes on recv (nic)",self.handle);
                                        tcp_socket.register_recv_waker(waker);
                                    }
                                    if wake == WakeOn::Send || wake == WakeOn::SendRecv {
                                        trace!("AsyncTcpSocket with{:?} is Pending and wakes on send (nic)",self.handle);
                                        tcp_socket.register_send_waker(waker);
                                    }
                                })
                            );
	...
	{
		nic::lock().with(|nic| nic.with_tcp_socket_ref(self.handle, f))
	}
	...
	pub(crate) fn is_active(&self) -> bool {
		self.with_socket_ref(|socket| socket.is_active())
	}

	/// Not in CLOSED or TIME-WAIT state
	pub(crate) fn is_open(&self) -> bool {
		self.with_socket_ref(|socket| socket.is_open())
	}

	/// In LISTEN state
	pub(crate) fn is_listening(&self) -> bool {
		self.with_socket_ref(|socket| socket.is_listening())
	}

	/// In ESTABLISHED state
	pub(crate) fn is_connected(&self) -> bool {
		self.with_socket_ref(|socket| socket.state() == TcpState::Established)
	}

	/// has data is receive buffer and may recv data
	pub(crate) fn can_recv(&self) -> bool {
		self.with_socket_ref(|socket| socket.can_recv())
	}

	/// send buffer not full and can send
	pub(crate) fn can_send(&self) -> bool {
		self.with_socket_ref(|socket| socket.can_send())
	}

	/// connect this socket to a remote using the local address
	///
	/// the socket must not be open
	pub(crate) fn connect(&mut self, remote: IpEndpoint) -> io::Result<()> {
		if !remote.is_specified() || remote.port == 0 {
			Err(io::Error::new(
				io::ErrorKind::InvalidData,
				"can't connect. invalid remote",
			))
		} else {
			if self.local.port == 0 {
				self.local.port = local_endpoint();
			}
			let local = self.local;
			match self.update_state() {
				State::Closed if self.is_open() => Err(io::Error::new(
	...
	pub(crate) fn listen(&mut self) -> io::Result<()> {
		if self.local.port == 0 {
			return Err(io::Error::new(
				io::ErrorKind::InvalidData,
				"can't listen on unspecified port",
			));
		}
		match self.update_state() {
		...
		let local = self.local;
		self.with_socket_mut(|socket| socket.listen(local))
			.map_err(|err| match err {
				smoltcp::Error::Illegal => io::Error::new(io::ErrorKind::Other, "already open"),
				smoltcp::Error::Unaddressable => {
					io::Error::new(io::ErrorKind::InvalidInput, "port unspecified")
				}
	...
	pub(crate) fn accept(&mut self) -> io::Result<IpEndpoint> {
		match self.update_state() {
			State::Connected => self.with_socket_mut(|socket| {
			...
			State::Listening => Err(io::Error::new(
				io::ErrorKind::WouldBlock,
	...
	pub(crate) fn wclose(&mut self) {
		...
		executor::spawn(async move {
			...
			async_socket.with_socket_mut(|socket| socket.close());
		})
		.detach();
	}
	...
	pub(crate) fn read(&mut self, buffer: &mut [u8]) -> io::Result<usize> {
		match self.update_state() {
			State::Connected | State::WClosed => self.with_socket_mut(|socket| {
				if socket.can_recv() {
					socket
						.recv_slice(buffer)
	...
	pub(crate) fn write(&mut self, buffer: &[u8]) -> io::Result<usize> {
		match self.update_state() {
			State::Connected | State::RClosed => self.with_socket_mut(|socket| {
				if socket.can_send() {
					socket
						.send_slice(buffer)
	...
	pub(crate) async fn wait_for_writeable(&self) {
		self.poll_with_socket(|socket, wake| {
			trace!("polling wait for writeable");
			if !socket.may_send() {
				Poll::Ready(())
	...
	pub(crate) async fn wait_for_incoming_connection(&self) {
		self.poll_with_socket(|socket, wake| {
			trace!(
				"polling for incoming connection in state: {:?}",
				socket.state()
			);
			if !socket.is_open() {
				Poll::Ready(())
...
impl From<SocketHandle> for AsyncTcpSocket {
	fn from(handle: SocketHandle) -> Self {
		...
		let socket = Self {
			...
			state: SharedState::new(if is_active {
				State::Connected
			} else {
				State::Closed
			}),
...
impl Drop for AsyncTcpSocket {
	fn drop(&mut self) {
		trace!("dropping async tcp socket");
