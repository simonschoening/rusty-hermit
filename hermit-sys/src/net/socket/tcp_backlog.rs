...
impl AsyncTcpBacklog {
    pub(crate) fn new(socket: Socket, capacity: usize, local: IpEndpoint) -> Self {
        let mut backlog = Vec::with_capacity(capacity);
        nic::lock().with(|nic| {
            for _ in 0..capacity {
                let handle = nic.create_tcp_handle();
                let socket = AsyncTcpSocket::new(Some(socket),handle,local);
                backlog.push(socket);
            }
        });
        Self { backlog, socket, local }
    }
        ...
        self.backlog
            ...
            .find_map(|async_socket| {
                if let Ok(_) = async_socket.accept() {
                    ...
                    let async_socket = std::mem::replace(async_socket, 
